...
function getInitialLetter(name, email) {
  return (
    normalize(name || email).charAt(0).toUpperCase() || "T"
  );
}
...
function getInviteEmptyMessage(status) {
  if (status === "PENDING") {
    return "No pending teacher invitations.";
  }

  if (status === "USED") {
    return "No used teacher invitations.";
  }

  if (status === "REVOKED") {
    return "No revoked teacher invitations.";
  }
...
function TeacherCard({
  ...
  const teacherName = row.name || "Unnamed teacher";
    ...
    <section className="bg-transparent">
      <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Invite teachers by email and manage existing invitations.
          </p>
        </div>
        ...
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-500">
              Add the teacher email. The teacher can use Member
...
export default function AdminTeachers() {
  ...
  const teacherInvites = useMemo(() => {
    return invites.filter(
      (invite) => invite.user_role === "teacher"
    );
  }, [invites]);
  ...
  const filteredSortedRows = useMemo(() => {
    ...
    return [...rows].sort(
      (firstRow, secondRow) => {
        const firstValue =
          getSortValue(firstRow);

        const secondValue =
          getSortValue(secondRow);
  ...
  const handleGrant = async (row) => {
    ...
    try {
      ...
      window.alert(
        getApiMessage(
          error,
          "Could not grant teacher access."
        )
      );
    } finally {
  ...
  const handleRevoke = async (row) => {
    ...
    const confirmed = window.confirm(
      `Revoke teacher access for "${row.email}"?`
    );
    ...
    try {
      ...
      window.alert(
        getApiMessage(
          error,
          "Could not revoke teacher access."
        )
      );
    } finally {
  ...
  const handleCreateInvite = async (event) => {
    ...
    try {
      ...
      setInviteError(
        getApiMessage(
          error,
          "Could not create teacher invitation."
        )
      );
    }
  };
    ...
    revokeInviteMutation.mutate(invite.id, {
      onError: (error) => {
        setInviteListError(
          getApiMessage(
            error,
            "Could not revoke teacher invitation."
          )
        );
      },
    });
  ...
  const handleReinviteInvite = async (
    ...
    try {
      ...
      setInviteListError(
        getApiMessage(
          error,
          "Could not reactivate teacher invitation."
        )
      );
    }
  };
  ...
  return (
    <>
      <main className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-transparent">
        ...
        {/* Doar lista principală are scroll */}
