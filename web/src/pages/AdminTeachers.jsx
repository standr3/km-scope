...
function getInitialLetter(name, email) {
  return normalize(name || email).charAt(0).toUpperCase() || "T";
}
...
function getInviteEmptyMessage(status) {
  if (status === "PENDING") return "No pending teacher invitations.";
  if (status === "USED") return "No used teacher invitations.";
  if (status === "REVOKED") return "No revoked teacher invitations.";

  return "No teacher invitations found.";
}

function buildTeacherRows(requests, teachers) {
  ...
  const grantedRows = teachers.map((teacher) => ({
    kind: "granted",
    id: `membership:${teacher.membership_id}`,
...
function TeacherCard({ row, isBusy, onGrant, onRevoke }) {
...
function TeacherInviteCard({ invite, isBusy, onRevoke, onReinvite }) {
    ...
    <section className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3">
      <div className="mb-3 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Invite teachers by email. They can set up their account from Member Login.
          </p>
        </div>
        ...
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-500">
              Add the teacher email. No email is sent yet; the teacher can use
...
export default function AdminTeachers() {
  ...
  const requestsTeachers = data.requests_teachers ?? [];
  const teachers = data.teachers ?? [];

  const invites = invitesQuery.data ?? [];
  const teacherInvites = useMemo(() => {
    return invites.filter((invite) => invite.user_role === "teacher");
  }, [invites]);
  ...
  const pendingCount = requestsTeachers.length;
  const grantedCount = teachers.length;
  const pendingInviteCount = teacherInvites.filter(
  ...
  const allRows = useMemo(() => {
    return buildTeacherRows(requestsTeachers, teachers);
  }, [requestsTeachers, teachers]);
  ...
  const handleRevoke = (row) => {
    ...
    if (!window.confirm(`Revoke teacher access for "${row.email}"?`)) {
      return;
    }
  ...
  const handleCreateInvite = async (event) => {
    ...
    try {
      ...
      setInviteError(
        getApiMessage(error, "Could not create teacher invitation.")
      );
    }
  };
  ...
  const handleRevokeInvite = (invite) => {
    ...
    revokeInviteMutation.mutate(invite.id, {
      onError: (error) => {
        setInviteListError(
          getApiMessage(error, "Could not revoke teacher invitation.")
        );
      },
    });
  };
  ...
  const handleReinviteInvite = async (invite) => {
    ...
    try {
      ...
      setInviteListError(
        getApiMessage(error, "Could not reactivate teacher invitation.")
      );
    }
  };
    ...
    return (
      <main className="h-full min-h-0 overflow-hidden bg-slate-50">
        <section className="grid h-full min-h-0 place-items-center rounded-2xl bg-white p-3 shadow-sm sm:p-4">
          <p className="text-sm text-slate-500">Loading teacher accounts...</p>
        </section>
      </main>
    );
  }
            ...
            <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
              <div className="min-w-0">
                ...
                <p className="mt-1 line-clamp-1 text-xs text-slate-500 sm:text-sm">
                  Review pending teacher requests, manage existing teacher access and create invitations.
                </p>
              </div>
              ...
              <div className="grid h-full min-h-[260px] place-items-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center">
                <div>
                  ...
                  <p className="mt-1 max-w-sm text-sm text-slate-500">
                    Adjust the search or filter to see teacher accounts.
                  </p>
                </div>
              </div>
            )}
